...
hns_uio_probe(struct platform_device *dev)
{
    ...
    /* get port-id */
    err = device_property_read_u32(&dev->dev, "port-idx-in-ae", &port);
    if (err){
        err = device_property_read_u32(&dev->dev, "port-id", &port);
        if(err)
            goto fail_free;
    }
    printk(KERN_DEBUG "port-id: %d\n",port);

    /* remap IO memory */
    /* seems this device has no resources */
    ...
    if(err != 0)
        dev_err(&dev->dev,"set dma failed!");
//      goto fail_release_iomem;
