...
static struct rte_trie_node * zmalloc_node(const char* name, int num, int socket_id){
	
	return rte_zmalloc_socket(name, (sizeof(struct rte_trie_node))*num,
...
rte_trie_create(const char *name, int socket_id)
{
	...
	trie->trie_node = (struct rte_trie_node *)rte_zmalloc_socket("TRIE_NODE", sizeof(struct rte_trie_node),
			RTE_CACHE_LINE_SIZE, socket_id);
...
int
rte_trie_add(struct rte_trie *trie, uint32_t ip, uint8_t depth, uint32_t next_hop, int socket_id)
{
	char mem_name[RTE_LPM_NAMESIZE];
	...
	if(node->next == NULL){
		node->next = zmalloc_node("TRIE_NODE_ROOT", RTE_NODE_NUM,socket_id);
		if(node->next ==NULL){
			RTE_LOG(ERR, LPM, "LPM trie_node memory allocation failed\n");
			goto exit;
		}
	...
	if(depth <= 8){
		rte_trie_add_help(node, index,depth,next_hop);
		return 0;
	}
	depth -=8;
	...
	if(node->next == NULL){
		snprintf(mem_name, sizeof(mem_name), "TRIE_NODE_%x", ip>>24);
		node->next = zmalloc_node("TRIE_NODE_ROOT", RTE_NODE_NUM,socket_id);
		if(node->next ==NULL){
			RTE_LOG(ERR, LPM, "LPM trie_node memory allocation failed\n");
			goto exit;
	...
	if(node->next == NULL){
		snprintf(mem_name, sizeof(mem_name), "TRIE_NODE_%x", ip>>16);
		node->next = zmalloc_node("TRIE_NODE_ROOT", RTE_NODE_NUM,socket_id);
		if(node->next ==NULL){
			RTE_LOG(ERR, LPM, "LPM trie_node memory allocation failed\n");
			goto exit;
	...
	if(node->next == NULL){
		snprintf(mem_name, sizeof(mem_name), "TRIE_NODE_%x", ip>>8);
		node->next = zmalloc_node("TRIE_NODE_ROOT", RTE_NODE_NUM,socket_id);
		printf("malloc:%p\n",node->next);
		if(node->next ==NULL){
			RTE_LOG(ERR, LPM, "LPM trie_node memory allocation failed\n");
...
int rte_trie_node_print(struct rte_trie_node *node){
	if(node){
		if(node->next){
			int i;
			for(i=0;i<RTE_NODE_NUM;i++){
				if((node->next[i]).valid){
					printf("%d,",i);
					rte_trie_node_print(&(node->next[i]));
					printf("\n");
					continue;
				}
				rte_trie_node_print(&(node->next[i]));
...
int rte_trie_print(struct rte_trie *trie){
	if(trie)
	{
		rte_trie_node_print(trie->trie_node);
	}
	return 0;
}
...
int rte_trie_node_free(struct rte_trie_node *node){
	if(node){
		if(node->next){
			int i;
			for(i = 0; i<RTE_NODE_NUM;i++){
				rte_trie_node_free(&(node->next[i]));
			}
			// printf("free %p\n",node->next);
			rte_free(node->next);
		}
...
int rte_trie_free(struct rte_trie *trie){
	printf("==========rte_trie_free=========\n");
    if(trie)
	{
		rte_trie_node_free(trie->trie_node);
